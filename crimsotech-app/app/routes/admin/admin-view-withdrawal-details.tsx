...
export async function loader({
  ...
  try {
    ...
    if (response.data?.success) {
      withdrawalDetail = response.data.withdrawal || response.data.data;
    } else {
    ...
    if (err.response?.status === 404) {
      error =
        "Withdrawal endpoint not found. Please check if the backend API is properly configured.";
    } else {
...
// Action Dialog Components using AlertDialog
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const response = await AxiosInstance.post(
        `/admin-withdrawals/${withdrawal.id}/approve/`,
        {},
        { headers },
      );
      ...
      if (response.data?.success) {
        ...
        toast({
          title: "Error",
          description: response.data?.error || "Failed to approve withdrawal",
          variant: "destructive",
        });
      }
    } catch (error: any) {
      toast({
        title: "Error",
        description:
          error.response?.data?.error || "Failed to approve withdrawal",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
      ...
      <AlertDialogContent>
        ...
        <div className="py-4">
          <div className="space-y-2">
            <p>
              <strong>Withdrawal ID:</strong> {withdrawal.withdrawal_id}
            </p>
            <p>
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        toast({
          title: "Error",
          description: response.data?.error || "Failed to reject withdrawal",
          variant: "destructive",
        });
      }
    } catch (error: any) {
      toast({
        title: "Error",
        description:
          error.response?.data?.error || "Failed to reject withdrawal",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
...
const ProcessDialog = ({
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const response = await AxiosInstance.post(
        `/admin-withdrawals/${withdrawal.id}/process/`,
        {},
        { headers },
      );
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent>
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction onClick={handleProcess} disabled={loading}>
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (response.data?.success) {
        ...
        toast({
          title: "Error",
          description: response.data?.error || "Failed to complete withdrawal",
          variant: "destructive",
        });
      }
    } catch (error: any) {
      toast({
        title: "Error",
        description:
          error.response?.data?.error || "Failed to complete withdrawal",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="sm:max-w-md">
        <AlertDialogHeader>
          <AlertDialogTitle>Complete Withdrawal</AlertDialogTitle>
          <AlertDialogDescription>
...
export default function ViewWithdrawalDetails({
  loaderData,
}: {
  loaderData: LoaderData;
}) {
  const { user, userId, withdrawal, error } = loaderData;
  const navigate = useNavigate();
  const [dialogType, setDialogType] = useState<
    "approve" | "reject" | "process" | "complete" | null
  >(null);
  ...
  const userInitials =
    withdrawal.user?.username?.substring(0, 2).toUpperCase() || "U";

  // Helper function to get image URL (handles S3 URLs if needed)
  ...
  const proofImageUrl = withdrawal.admin_proof ? getImageUrl(withdrawal.admin_proof) : null;
      ...
      <div className="container mx-auto p-4 md:p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Proof of Payment Card - Now shows image directly */}
            {withdrawal.admin_proof && proofImageUrl && (
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div className="rounded-lg border overflow-hidden bg-gray-50">
                      {proofImageUrl.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                        <img
                          src={proofImageUrl}
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <InfoRow
                  ...
                  value={
                    <CopyableText
                      text={withdrawal.wallet?.wallet_id || "N/A"}
                      label="Wallet ID"
                    />
                  }
                  icon={Wallet}
                ...
                <InfoRow
                  ...
                  value={formatCurrency(withdrawal.wallet?.balance || 0)}
                  icon={PhilippinePeso}
                />
                ...
                {withdrawal.wallet?.available_balance !== undefined && (
                  <InfoRow
                    ...
                    value={formatCurrency(withdrawal.wallet.available_balance)}
                    icon={PhilippinePeso}
                  />
                )}
                ...
                {withdrawal.wallet?.pending_balance !== undefined && (
                  <InfoRow
                    ...
                    value={formatCurrency(withdrawal.wallet.pending_balance)}
                    icon={Clock}
                  />
                )}
              </CardContent>

...
export default function FileDisputeClient({ loaderData }: Route.ComponentProps) {
  ...
  const fetchRefund = useCallback(async () => {
    ...
    try {
      ...
      setOrderId(data?.order_info?.order_id || null);
      setOrderNumber(data?.order_info?.order_number || null);
    } catch (e: any) {
      toast({
        title: 'Error',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/disputes/file_dispute/`, {
        method: 'POST',
      ...
      if (files.length > 0 && data?.dispute_id) {
        ...
        const uploadPromises = files.map(async (file) => {
          ...
          try {
            ...
            const evidenceRes = await fetch(
              `${API_BASE_URL}/disputes/${data.dispute_id}/upload_evidence/`,
              {
                method: 'POST',

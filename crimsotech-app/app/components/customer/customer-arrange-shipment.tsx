// app/routes/customer-arrange-shipment.tsx
import type { Route } from './+types/customer-arrange-shipment';
...
export function meta(): Route.MetaDescriptors {
  return [
    {
      title: "Arrange Shipment - Personal Listing",
    },
  ];
}
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  try {
    ...
    return {
      ...
      user: {
        id: userId,
        name: orderResponse.data.data.seller_info.name,
        ...
        isRider: false,
        isModerator: false,
        username: orderResponse.data.data.seller_info.username,
      },
...
const deliveryMethods: DeliveryMethod[] = [
  {
    ...
    description: "Seller delivers directly to buyer's address",
    icon: <Truck className="w-8 h-8" />,
    cost: 0.00,
    estimated_days: "Flexible (seller sets timeline)",
    system_handles: [
      "Provides seller with buyer's address",
...
export default function CustomerArrangeShipment() {
  ...
  const { order, user, riders, orderId, userId } = loaderData;
    ...
    try {
      if (selectedMethod === "choose_rider") {
        ...
        if (offerData.success) {
          ...
          successMessage += "\n\nYou can track this in your 'Processing' orders.";

          alert(successMessage);
          navigate(`/order-list?tab=processing`);
        } else {
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-bold">Arrange Shipment - Personal Listing</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="p-3 border rounded-lg">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-medium">Seller (You)</span>
                    </div>
                    <div className="text-sm">
                      {order.seller_info.name}
                    </div>
                    <div className="text-xs text-gray-500">{order.seller_info.username}</div>
                    <div className="text-xs text-gray-500">{order.seller_info.phone}</div>
                    <div className="text-xs text-gray-500 mt-1">{order.seller_info.address}</div>
                  </div>
                  ...
                  <Label htmlFor="terms" className="text-xs leading-tight cursor-pointer">
                    ...
                    {selectedMethod === "choose_rider" && (
                      <span className="block mt-1 text-gray-600">
                        ...
                        • Final delivery fee will be added to order<br/>

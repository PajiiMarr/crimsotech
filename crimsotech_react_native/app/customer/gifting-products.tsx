...
export default function GiftingProductPage() {
      ...
      if (response.data) {
        ...
        if (productsData.length > 0) {
          console.log("Sample product from API:", {
            ...
            media_files_sample: productsData[0].media_files
              ? productsData[0].media_files[0] || productsData[0].media_files
              : undefined,
          ...
          for (const c of rawCandidates) {
            ...
            if (typeof c === "object") {
              ...
              const nested =
                ...
                c.file_url ||
                c.raw_url ||
                (c.file_data && (c.file_data.url || c.file_data.file)) ||
                c.file ||
                c.thumbnail;
        ...
        const transformedProducts = giftProducts.map((product: any) => {
          ...
          if (!rawImage) {
            console.warn("No image found in product payload", product.id, {
            ...
            console.log(
              "Resolved image candidate for product",
              product.id,
              rawImage,
            );
          }
  ...
  const getStockColor = (status: string, quantity: number) => {
    if (quantity === 0) return "#9CA3AF";
    if (status?.toLowerCase().includes("low")) return "#4B5563";
    return "#111827";
  };
    ...
    <TouchableOpacity style={styles.productCard} activeOpacity={0.7}>
      <View style={[styles.imageContainer, { backgroundColor: "#FFF5F5" }]}>
        <Image
          ...
          onError={(e: any) =>
            console.warn(
              "Failed to load image for product",
              item.id,
              item.primary_image,
      ...
      <View style={styles.productInfo}>
        <View style={styles.textContainer}>
          ...
          <Text style={styles.brandName}>{item.customer.name}</Text>

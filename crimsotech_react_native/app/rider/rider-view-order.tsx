...
export default function RiderViewOrder() {
  ...
  // ========== CRITICAL: BUTTON SET LOGIC - ONLY ONE SET PER DELIVERY ==========
  const deliveryStatus = orderDetails.delivery?.status;
  ...
  // Determine which button set to show (ONLY ONE will be true)
  let buttonSet = null; // 'pending', 'accepted', 'transit', 'delivered', 'waiting', or null

  if (deliveryStatus === 'delivered') {
    buttonSet = 'delivered';
  } else if (deliveryStatus === 'picked_up') {
    buttonSet = 'transit';
  } else if (deliveryStatus === 'accepted' && !isReturnWaitingForBuyer) {
    buttonSet = 'accepted';
  } else if (deliveryStatus === 'accepted' && isReturnWaitingForBuyer) {
    buttonSet = 'waiting';
  } else if (deliveryStatus === 'pending' || deliveryStatus === 'pending_offer') {
    buttonSet = 'pending';
  }

  const statusMessage = getStatusMessage(deliveryStatus);
  const isAccepted = deliveryStatus === 'accepted';
  const statusColor = getStatusColor(deliveryStatus);
  const statusLabel = getStatusLabel(deliveryStatus);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F9FAFB' }}>
      ...
      >
                  ...
                  <View style={{ backgroundColor: '#FFFBEB', borderRadius: 10, padding: 12, borderWidth: 1, borderColor: '#FDE68A' }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                      ...
                      <Text style={{ fontSize: 12, color: '#92400E', marginLeft: 6 }}>
                        Failed on {formatDate(orderDetails.delivery?.created_at)}
                      </Text>
                    </View>
        ...
        {/* Order/Refund Items */}
        <View style={{ backgroundColor: '#FFFFFF', borderBottomWidth: 1, borderTopWidth: 1, borderColor: '#F3F4F6', marginBottom: buttonSet ? 0 : 80, marginTop: -1 }}>
          <View style={{ padding: 16 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
      ...
      {/* ========== ACTION BUTTONS - ONLY ONE SET PER DELIVERY ========== */}
      ...
      {/* ACCEPTED Status - Cancel and Mark as Picked Up Buttons */}
      {buttonSet === 'accepted' && (
        <View style={{ 
          backgroundColor: '#FFFFFF', 
          borderTopWidth: 1, 
          ...
          <View style={{ flexDirection: 'row', gap: 12 }}>
            ...
            <TouchableOpacity
              ...
              disabled={isActionLoading || isShopStatusRiderAssigned}
              style={{
                flex: 1,
                backgroundColor: isShopStatusRiderAssigned ? '#9CA3AF' : '#3B82F6',
                paddingVertical: 14,
                borderRadius: 0,
                marginRight: 16,
                opacity: isShopStatusRiderAssigned ? 0.7 : 1,
              }}
            >
              <Text style={{ 
                fontSize: 16, 
      ...
      {/* IN TRANSIT Status - View Route, Failed, Delivered Buttons */}
      {buttonSet === 'transit' && (
        <View style={{ 
          backgroundColor: '#FFFFFF', 
          borderTopWidth: 1, 
      ...
      {/* WAITING FOR BUYER Status (Return Delivery Only) - No Action Buttons, Just Info */}
      {buttonSet === 'waiting' && (
        <View style={{ 
          backgroundColor: '#FFFFFF', 
          borderTopWidth: 1, 
          ...
          <View style={{ 
            ...
            <Text style={{ fontSize: 14, color: '#92400E', marginLeft: 8, flex: 1 }}>
              Waiting for buyer to mark item as ready for pickup...
            </Text>
          </View>
        </View>
      )}
